...
public class ParticleSnowForDraw
{
    ...
    String mFragmentShader;//片元着色
    FloatBuffer mVertexBuffer;//顶点坐标数据缓冲
    FloatBuffer mTexCoorBuffer;//顶点纹理坐标数据缓冲
    int vCount=0;   //顶点个数
    float halfSize;//粒子半径
    public ParticleSnowForDraw(GameSurfaceView mv, float halfSize)
    {//构造器
    ...
    {
        //加载顶点着色器的脚本内容
        mVertexShader= ShaderUtil.loadFromAssetsFile("snowVertex.sh", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("snowFrag.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
    ...
    public void drawSelf(int texId,float maxLifeSpan)
    {
        //制定使用某套shader程序
